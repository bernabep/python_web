...
def obtener_web_masvoz(seccion='Estadisticas',maximizado=False):
    """
    Puedes elegir la seccion en la cua3h4hl abrir la web de MasVoz, [inicio,agenda,skill,supervision,estadisticas,detalles,cuenta]
    """
    ...
    elemento = esperar_elemento(driver= driver, buscar_por= By.CLASS_NAME, texto_buscado= class_elemento, accion= 'click',tiempo_espera=5)
...
def robot_informes_masvoz(fecha_hora_inicio=datetime.now()-timedelta(minutes=30),lista_informes=lista_informes_a_sacar):
    ...
    existe_archivo = os.path.exists(os.path.join(ruta_destino,'informe_skills_de_agentes.csv'))
    fecha_hora_primer_tramo = datetime.combine(datetime.today(), time(primer_tramo_del_dia, 0, 0))
...
def iniciarRobot():
    try:    
        ejecutar_en_minutos(funcion_a_lanzar=robot_informes_masvoz,lista_minutos=[15,45])
