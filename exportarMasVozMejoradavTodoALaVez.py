...
segundos_de_espera = 6 #!Dependiendo del PC, los segundos de espera tienen que aumentarse, afecta sobretodo al navegar por la web
...
def robot_informes_masvoz(fecha_hora_inicio=datetime.now()-timedelta(minutes=30),lista_informes=lista_informes_a_sacar):
    ...
    if any(item in lista_informes for item in ['colas', 'tramos','colas_tramos','actividad_por_agente','actividad_por_agente_cola','estados_por_agente']):
        ...
        if(tipoInforme in lista_informes):
            mensaje = f"### Empezando a sacar el informe_{tipoInforme} del {fechaDatos} {hora_inicio} ###"
...
def iniciarRobot():
    try:    
        ...
        ejecutar_en_minutos(funcion_a_lanzar=lanzar_varios_robot_informes_masvoz,lista_minutos=[1,31])
...
def lanzar_varios_robot_informes_masvoz():
    ...
    proceso_detalles = threading.Thread(target=robot_informes_masvoz,args=(datetime.now()-timedelta(minutes=30),['listado_llamadas','listado_acd']))
    proceso_usuarios_masvoz = threading.Thread(target=robot_informes_masvoz,args=(datetime.now()-timedelta(minutes=30),['usuarios_masvoz']))
    ...
    proceso_acumular = threading.Thread(target=exportar_tramos_faltantes,args=([['colas','tramos','colas_tramos','actividad_por_agente','actividad_por_agente_cola','estados_por_agente','agentes','listado_llamadas','listado_acd']]))
    
    lista_procesos = {proceso_colas,proceso_agentes,proceso_actividad_agentes,proceso_estados_agente,proceso_detalles,proceso_usuarios_masvoz,proceso_skills_agentes}
    ...
    proceso_acumular_detalles = threading.Thread(target=exportar_tramos_faltantes,args=([['listado_llamadas','listado_acd']]))
    # proceso_acumular_usuarios_masvoz = threading.Thread(target=robot_informes_masvoz,args=([['usuarios_masvoz']]))
    # proceso_acumular_skills_agentes = threading.Thread(target=robot_informes_masvoz,args=([['skills_agentes']]))
    
    lista_procesos = {proceso_acumular_colas,proceso_acumular_agentes,proceso_acumular_actividad_agentes,proceso_acumular_estados_agente,proceso_acumular_detalles}
