...
lista_informes_a_sacar=['colas','tramos','colas_tramos','actividad_por_agente','actividad_por_agente_cola','estados_por_agente','agentes','listado_llamadas','listado_acd','usuarios_masvoz']
...
num_dias_para_acumular_colas_tramos=0 #!Importante, si se ponen menos días, se eliminarán del acumulado los días que no cumplen la condición
...
def obtener_web_masvoz(seccion='Estadisticas',maximizado=False):
    ...
    """
    seccionesDisponibles = ['inicio','agenda','skill','supervision','estadisticas','detalle','cuenta']
            ...
            for columna in columnas:
                match posicion_columna:
                    ...
                    case 3: 
                        ...
                        nombre_apellido,usuario,email,estado = '','','',''
                        posicion_columna=0
                
            ruta_elemento = f'/html/body/div[6]/div/div/div[3]/div[1]/div/div/div/div[2]/div/table/tbody/tr[{index}]'
            elemento = esperar_elemento(driver=driver,buscar_por=By.XPATH,texto_buscado=ruta_elemento,tiempo_espera=3)
            if elemento is None:
                llego_al_final = True
                break
...
def insertar_columna_csv(archivo_origen,nombre_columna,dato,separacion=';',intentos=0):
    while (intentos<10):
        try:
        # Leer el archivo CSV en un DataFrame
            try:
            ...
            df.to_csv(archivo_origen,sep=separacion,encoding="utf-8")
        except PermissionError:
            intentos +=1
            print(f'El archivo {archivo_origen} está bloqueado...intentos {intentos} de 10')
            sleep(3)
            insertar_columna_csv(archivo_origen=archivo_origen,nombre_columna=nombre_columna,separacion=separacion,intentos=intentos)
...
def ejecutar_en_minutos(funcion_a_lanzar,lista_minutos=[1,31]):
    ...
    while True:
        ...
        if(horaInicial.minute in lista_minutos) or primeraVuelta:
            ...
            funcion_a_lanzar(fecha_hora_inicio=fecha_hora_inicio,lista_informes=lista_informes_a_sacar)
...
def robot_informes_masvoz(fecha_hora_inicio=datetime.now()-timedelta(minutes=30),lista_informes=lista_informes_a_sacar):
    ...
    """
    IMPORTANTE Esto busca y exporta los usuarios de masvoz, solo se saca una vez por la mañana en el primer tramo
    """  
    
    
    if fecha_hora_primer_tramo == fecha_hora_inicio_tramo:
        try:
            descargar_usuarios_masvoz(driver_masvoz)
        except Exception as e:
            print(f'Hubo un error en la funcion robot_informes_masvoz, pricisamente en la parte que llama a la funcion descargar_usuarios_masvoz, el erros es:\n{e}')
       
           
       
    fecha_hora_primer_tramo = datetime.today().date()+timedelta(hours=primer_tramo_del_dia)
    if any(item in lista_informes for item in ['usuarios_masvoz']) and fecha_hora_primer_tramo == fecha_hora_inicio_tramo:
        print(f'Abriendo web MasVoz')
    ...
    """  
    exportar_tramos_faltantes(lista_informes=['colas','tramos','colas_tramos','actividad_por_agente','actividad_por_agente_cola','estados_por_agente','agentes','listado_llamadas','listado_acd'])
