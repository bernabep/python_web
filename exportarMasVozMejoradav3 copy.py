...
silencioso=False #!Para que no muestre tantos print, hay que poner True
...
num_dias_para_acumular_colas_tramos=2 #!Importante, si se ponen menos días, se eliminarán del acumulado los días que no cumplen la condición
...
def esperar_iframe(driver,buscar_por,texto_buscado,accion='', texto_a_escribir= '',primero_borrar= False,tiempo_espera=30):
    elemento = None
    wait = WebDriverWait(driver,tiempo_espera)
    condicion = EC.frame_to_be_available_and_switch_to_it((buscar_por,texto_buscado))
    hora_inicio = datetime.now()
    tiempo_delta = timedelta(seconds=tiempo_espera)
    hora_fin = hora_inicio + tiempo_delta
...
def obtener_web_masvoz(seccion='Estadisticas'):
    """
...
def obtener_tramos_faltantes_bd(tabla,fecha_inicio,fecha_fin,columna_fecha,columna_tiempo,ultimo_tramo_del_dia=False,minutosPorTramo=30):
    ...
    if (len(lista_fechas_tramos_faltantes)>0):
        ...
        for fecha in lista_fechas_tramos_faltantes:
            print(f'{fecha}')
    else:
...
def obtener_usuarios_masvoz():
    guardado = False
    try:
        driver = obtener_web_masvoz('Cuenta')
        ...
        print(f'Hubo un error en la funcion obtener_usuarios_masvoz, el erros es:\n{e}')
    else:
